...
You are testing whether the skill works in a fresh opencode agent.

Rules:
- You may run commands and access the network.
- Do not modify project files or skill files.
- Use only {{ARTIFACT_DIR}} for generated artifacts.
- The test workspace contains symlinks at ./enji-fleet-browser and .agents/skills/enji-fleet-browser.
...
- Shell note: export SKILL_DIR before using "$SKILL_DIR/..." in a later command segment.
- Run captures sequentially: finish enji.ai first, then run clutch.co. Do not launch both helper commands in parallel.
